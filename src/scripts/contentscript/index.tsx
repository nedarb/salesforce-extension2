...
const RelationshipsAction: SelectItem = {
  value: 'relationships',
  label: 'Relationships',
};
...
async function getOptions(
  ...
  if (selection.length > 0) {
    const [selectedAction, selectedSubAction, selectedSubActionValue] =
      selection;
      ...
      case UpdateAction.value:
        ...
        if (selectedSubAction) {
          ...
          if (field) {
            ...
            const referenceTo =
              field.referenceTo?.length === 1 ? field.referenceTo[0] : null;
            if (referenceTo) {
              // query reference to
              console.log('Going to search referenceTo', field.referenceTo);
              const query =
                currentQuery.length > 0
                  ? `select Id, Name from ${referenceTo} where name like '%${currentQuery}%' limit 10`
                  : `SELECT Id, Name FROM RecentlyViewed WHERE Type='${referenceTo}' ORDER BY LastViewedDate DESC`;

              const lookupUrl = new URL(
                `/services/data/v55.0/ui-api/lookups/${sobjectName}/${field.name}/${referenceTo}?searchType=Recent`,
              ...
              const lookupResults =
                await apiCaller.makeApiCall<FieldLookupResult>(
                  lookupUrl.pathname + lookupUrl.search,
                );
              const lookupOptions = lookupResults.records.map((r) => {
                ...
                return {
                  value: r.id,
                  label: disambiguationField
                    ? `${label} (${disambiguationField.value})`
                    : label,
                };
              });
          ...
          return [
            UpdateAction,
            { value: selectedSubAction, label: selectedSubAction },
          ];
        }
        return [
          UpdateAction,
        ...
        if (selectedSubAction) {
          ...
          return [
            ...
            ...describeResult.childRelationships
              .filter((r) => r.relationshipName === selectedSubAction)
              .map((r) => ({
    ...
    execute: async (makeApiCall: MakeApiCall, [, fieldName, updatedValue]) => {
      ...
      try {
        const result = await makeApiCall(
          `/services/data/v50.0/sobjects/${sobjectName}/${recordId}`,
          'PATCH',
...
function ToDisplay({
  ...
  const apiUrl = sobjectName
    ? `/services/data/v50.0/sobjects/${sobjectName}/describe`
    : undefined;
  const { results, isLoading } = useSalesforceApi<SObjectDescribeResult>({
    url: apiUrl,
    cookie,
    useCache: false,
  });
  const [query, setQuery] = useState('');
  const [debouncedQuery, immediatelySetDebouncedQuery] = useDebounce(
    query,
    500,
  );
  ...
  const [options, optionsLoading] = useAsyncState2(
    getOptions,
    apiCaller,
    sobjectName,
    value2,
